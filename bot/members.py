...
class Members(commands.Cog):
    ...
    async def on_user_update(self, old: discord.Member, new: discord.Member):
        if old.bot:  # User is bot - ignore to prevent TypeError NoneType
            self.logger.info(f"Skip bot account - {old.global_name} update username to {new.global_name}")
            pass
        else:
            previous_username = f"{old.global_name}"
            updated_username = f"{new.global_name}"
    ...
    async def on_member_update(self, old: discord.Member, new: discord.Member):
        if old.bot:  # User is bot - ignore to prevent TypeError NoneType
            self.logger.info(f"Skip bot account - {old.nick} update nickname to {new.nick}")
            pass
        else:
    ...
    async def on_member_join(self, new_member: discord.Member):
        ...
        search_result = None
        async for result in self.db.members.find(key):
            # print(f"Existing record for {new_member.global_name}"  # DEBUG
            #      f"found: ObjectID:{result['_id']}")

            search_result = result

        if search_result is not None:  # Database entry found!
            self.logger.debug(f"Updating existing record for {new_member.global_name}"
                              f"under: ObjectID:{search_result['_id']}")

            # Update Usernames
            current_full_username = str(f"{new_member.global_name}")  # For comparing usernames
            ...
            if search_result["discord_username"] == current_full_username:  # Check if username needs updating
                ...
                username_updated_value = {
                    "$set": {
                        "discord_username": f"{new_member.global_name}",
                        "past_usernames": {
                            "username": [
                                f"{search_result['discord_username']}",
                                f"{search_result['past_usernames']['username'][0]}",
            ...
            self.logger.debug(f"No record found for {new_member.global_name}; Creating new entry.")
            ...
            await self.db.members.insert_one(
                {
                    "discord_username": str(f"{new_member.global_name}"),
    ...
    async def socials(self, interaction: discord.Interaction.response):
        ...
        try:
            ...
            pass
        try:
            current_bandcamp_url = search_result['other_url']
        except KeyError:
            self.logger.debug(f"Members.cog: {interaction.user.global_name} has no Other URL.")
            pass

        mixed_urls: list = [current_soundcloud_url, current_bandcamp_url, current_other_url]
...
class Socials(discord.ui.Modal, title='Social URL Management'):
    def __init__(self, interaction: discord.Interaction.response, urls: list):
        ...
        self.other_user_url = discord.ui.TextInput(
            label="Other Artist URL",
            style=discord.TextStyle.short,
            placeholder="https://websitehere.com",
            default=f"{current_other_url}",
            required=False,
            min_length=1,  # https:// + .bandcamp.com
            max_length=128
        )

        self.add_item(self.soundcloud_user_url)
        self.add_item(self.bandcamp_user_url)
        self.add_item(self.other_user_url)
    ...
    async def on_submit(self, interaction: discord.Interaction.response):
        socials_updated_value = {
            "$set": {
                ...
                "other_url": str(self.other_user_url)
            }
        }
    ...
    async def on_timeout(self):
        self.logger.info(f"Modal was not responded to, continuing...")
        pass
